...
function PianoKey({ note, isBlack = false, isPressed = false, onPress, onRelease, }) {
    ...
    if (isBlack) {
        ...
        `, onMouseDown: handleMouseDown, onMouseUp: handleMouseUp, onMouseLeave: handleMouseLeave, onTouchStart: handleTouchStart, onTouchEnd: handleTouchEnd, style: {
    ...
    return (_jsx("button", { className: `
        ...
        transition-all duration-75 select-none touch-none
        ${isPressed ? "bg-gray-800 scale-95" : "hover:bg-gray-900"}
      `, onMouseDown: handleMouseDown, onMouseUp: handleMouseUp, onMouseLeave: handleMouseLeave, onTouchStart: handleTouchStart, onTouchEnd: handleTouchEnd, style: { touchAction: "none" } }));
}
function TaskPromptCard({ note }) {
    const prompt = taskPrompts[note];
    if (!prompt)
        return null;
    return (_jsx("div", { className: "mt-8 w-full max-w-md mx-auto", children: _jsxs("div", { className: "bg-black border-2 border-white rounded-lg p-4 animate-in fade-in duration-200", children: [_jsx("h3", { className: "text-white font-mono tracking-wider mb-2", children: prompt.title }), _jsx("p", { className: "text-gray-300 font-mono text-sm leading-relaxed", children: prompt.content })] }) }));
}
function CloudFBM() {
    ...
    useFrame((state) => {
        ...
        if (mesh.current) {
            ...
            if (analyser && dataArray.current) {
                const spectrum = dataArray.current;
                analyser.getByteFrequencyData(spectrum);
                const volume = spectrum.reduce((sum, value) => sum + value, 0) /
                    spectrum.length /
                    255;
                currentVolume = volume;
                const bassEnd = Math.floor(spectrum.length * 0.1);
                const midEnd = Math.floor(spectrum.length * 0.4);
                const bass = spectrum.slice(0, bassEnd).reduce((sum, value) => sum + value, 0) /
                    bassEnd /
                    255;
                const mid = spectrum
                    ...
                    (midEnd - bassEnd) /
                    255;
                const treble = spectrum.slice(midEnd).reduce((sum, value) => sum + value, 0) /
                ...
                uniforms.u_color1.value.set(0.08 + bass * 0.15, 0.12 + bass * 0.2, 0.24 + bass * 0.3);
                uniforms.u_color2.value.set(0.25 + mid * 0.2, 0.35 + mid * 0.25, 0.65 + mid * 0.2);
                uniforms.u_color3.value.set(0.55 + treble * 0.25, 0.75 + treble * 0.15, 0.95 + treble * 0.05);
                uniforms.u_color4.value.set(0.9 + intensity * 0.1, 0.95 + intensity * 0.05, 1.0);
            ...
            uniforms.u_resolution.value.set(200 * (typeof window !== "undefined" ? window.devicePixelRatio : 1), 200 * (typeof window !== "undefined" ? window.devicePixelRatio : 1));
        }
    });
    const vertexShader = `
    varying vec2 vUv;
  ...
  `;
    return (_jsxs("mesh", { ref: mesh, scale: [viewport.width, viewport.height, 1], children: [_jsx("planeGeometry", { args: [1, 1] }), _jsx("shaderMaterial", { args: [{ fragmentShader, vertexShader, uniforms }] })] }));
}
function Visualizer() {
    return (_jsx("div", { className: "w-[200px] h-[200px] border-2 border-white rounded-lg overflow-hidden touch-none", children: _jsx(Canvas, { camera: { position: [0, 0, 1] }, children: _jsx(CloudFBM, {}) }) }));
}
function Controls() {
    ...
    const handleKnobChange = (index, event) => {
        if (isDragging === index) {
            ...
            setSynthParams({ ...synthParams, [paramNames[index]]: value });
        }
    };
    const handleKnobStart = (index) => { setIsDragging(index); };
    ...
    return (_jsxs("div", { className: "w-[200px] h-[200px] flex flex-col justify-between touch-none", children: [_jsx("div", { className: "h-[100px] bg-black border-2 border-white rounded-lg p-2 flex justify-between items-center", children: knobValues.map((value, index) => (_jsx("div", { className: "relative flex items-center justify-center", children: _jsx("div", { className: "w-16 h-16 cursor-pointer select-none flex items-center justify-center touch-none", onMouseDown: () => handleKnobStart(index), onMouseMove: (e) => handleKnobChange(index, e), onMouseUp: handleKnobEnd, onMouseLeave: handleKnobEnd, onTouchStart: (e) => { e.preventDefault(); handleKnobStart(index); }, onTouchMove: (e) => { e.preventDefault(); handleKnobChange(index, e); }, onTouchEnd: (e) => { e.preventDefault(); handleKnobEnd(); }, style: { touchAction: "none" }, children: _jsx("div", { className: "w-12 h-12 border-2 border-white rounded-full bg-black relative", children: _jsx("div", { className: "absolute top-1 left-1/2 w-0.5 h-4 bg-white origin-bottom transform -translate-x-1/2", style: { transform: `translateX(-50%) rotate(${(value - 0.5) * 270}deg)` } }) }) }) }, index))) }), _jsx("div", { className: "h-[64px] bg-black border-2 border-white rounded-lg p-2 flex items-center", children: _jsx("div", { className: "w-full relative cursor-pointer select-none py-4 touch-none", "data-slider": true, onMouseDown: handleSliderStart, onMouseMove: handleSliderMove, onMouseUp: handleSliderEnd, onMouseLeave: () => handleSliderEnd(), onTouchStart: handleSliderStart, onTouchMove: handleSliderMove, onTouchEnd: handleSliderEnd, style: { touchAction: "none" }, children: _jsx("div", { className: "w-full h-1 bg-white rounded-full relative", children: _jsx("div", { className: "absolute top-1/2 w-4 h-4 bg-white rounded-full transform -translate-y-1/2 -translate-x-1/2 cursor-pointer border border-gray-300 shadow-sm", style: { left: `${synthParams.pitchBend * 100}%`, touchAction: "none" } }) }) }) })] }));
}
export default function MidiPianoApp() {
    ...
    return (_jsx(AudioContextProvider.Provider, { value: { audioContext, analyser, gainNode, synthParams, setSynthParams }, children: _jsxs("div", { className: "min-h-screen bg-black flex flex-col items-center justify-center relative touch-none select-none", style: {
